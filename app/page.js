...
export default function SchedulerPage() {
  ...
  // 2. Helper: Calculate End Time
  const getEndTime = (startTime, duration, lunch) => {
    if (!startTime || duration <= 0) return "";
    const [h, m] = startTime.split(':').map(Number);
    const totalMinutes = h * 60 + m + (duration + parseFloat(lunch)) * 60;
  ...
  const calendarData = useMemo(() => {
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      let hours = state.exceptions[dateKey] !== undefined ? state.exceptions[dateKey] : state.defaults[dayOfWeek];
      let start = state.startExceptions[dateKey] || state.startDefaults[dayOfWeek];
      let lunch = state.lunchExceptions[dateKey] || state.lunchDefaults[dayOfWeek];
  ...
  // 4. Capture Feature (Needs html2canvas globally or via import)
  const handleCapture = async () => {
    if (typeof window !== 'undefined' && window.html2canvas) {
      ...
      alert("캡처 라이브러리를 로딩 중입니다. 잠시 후 다시 시도해주세요.");
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 bg-slate-900/40 p-8 rounded-3xl border border-slate-800 shadow-2xl backdrop-blur-xl">
          <div className="space-y-2">
            <h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400 tracking-tight">
              TIME KEEPER <span className="text-sm font-normal text-slate-500 ml-2">v2.2</span>
            </h1>
            <input 
              value={state.name} 
          ...
          <div className="flex flex-col justify-center space-y-3">
            <div className="flex justify-between text-xs font-black text-slate-500 uppercase tracking-widest">
              ...
              <span className={calendarData.totalAccHours >= 80 ? 'text-emerald-400' : 'text-blue-400'}>
                {calendarData.totalAccHours.toFixed(1)} / 80.0h
              </span>
            </div>
          ...
          <div className="bg-blue-500/5 rounded-2xl border border-blue-500/10 p-4 flex flex-col items-center justify-center text-center">
            ...
            <div className="text-2xl font-black text-blue-400 tracking-tighter">
              ₩ {calendarData.totalWage.toLocaleString()}
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <button 
            ...
            className="px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl text-xs font-black uppercase tracking-widest transition-all border border-slate-700 flex items-center gap-2"
          >
        ...
        {viewMode === 'personal' ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8" ref={calendarRef}>
                  ...
                  {DAYS_KOREAN.map((day, idx) => (
                    <div key={day} className="flex items-center justify-between">
                      ...
                      <input 
                        ...
                        onChange={(e) => saveState({ defaults: { ...state.defaults, [idx]: parseFloat(e.target.value) || 0 }})}
            ...
            <main className="lg:col-span-9 space-y-4">
              <div className="flex justify-center items-center gap-8 mb-2">
                <button onClick={() => setCurrentDate(new Date(year, month - 1))} className="text-slate-500 hover:text-white transition-colors">←</button>
                <h2 className="text-2xl font-black tracking-tighter">{year}년 {month + 1}월</h2>
                <button onClick={() => setCurrentDate(new Date(year, month + 1))} className="text-slate-500 hover:text-white transition-colors">→</button>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {teamSchedules.map((m) => {
              const total = Object.values(m.exceptions || {}).reduce((a, b) => a + (typeof b === 'number' ? b : 0), 0);
              return (
                <div key={m.id} className="bg-slate-900/40 p-6 rounded-3xl border border-slate-800 space-y-4 hover:border-slate-600 transition-all">
                  <div className="flex justify-between items-center">
                    <span className="font-black text-slate-200">{m.name}</span>
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-tighter">₩{(total * HOURLY_WAGE).toLocaleString()}</span>
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between text-[10px] font-black text-blue-400/80">
                      <span>{total.toFixed(1)}h</span>
                      <span>{Math.round((total/80)*100)}%</span>
                    </div>
                    <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500" style={{ width: `${(total / 80) * 100}%` }} />
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
            <div className="bg-[#161b22] w-full max-w-sm rounded-[2.5rem] border border-slate-700 p-10 shadow-2xl space-y-8">
              <div className="text-center space-y-1">
                <h3 className="text-2xl font-black text-blue-400">Schedule Detail</h3>
              ...
              <form onSubmit={(e) => {
                ...
                const h = fd.get('type') === 'off' ? 0 : parseFloat(fd.get('hours'));
                saveState({
                ...
                <div className="flex gap-2 p-1.5 bg-slate-900 rounded-2xl border border-slate-800">
                  <label className="flex-1">
                    ...
                    <div className="text-center py-2.5 rounded-xl text-xs font-black cursor-pointer peer-checked:bg-blue-600 peer-checked:text-white text-slate-600 transition-all">근무</div>
                  </label>
                  <label className="flex-1">
                    <input type="radio" name="type" value="off" defaultChecked={selectedDay.hours === 0} className="peer hidden" />
                    <div className="text-center py-2.5 rounded-xl text-xs font-black cursor-pointer peer-checked:bg-red-600 peer-checked:text-white text-slate-600 transition-all">휴무</div>
                  </label>
                </div>
                ...
                <div className="flex gap-4 pt-4">
                  <button type="button" onClick={() => setSelectedDay(null)} className="flex-1 py-4 rounded-2xl bg-slate-800 font-black text-xs uppercase tracking-widest hover:bg-slate-700 transition-all">Cancel</button>
                  <button type="submit" className="flex-1 py-4 rounded-2xl bg-blue-600 font-black text-xs uppercase tracking-widest hover:bg-blue-500 shadow-xl shadow-blue-900/30 transition-all text-white">Save Schedule</button>
                </div>
              </form>
            </div>
          </div>
